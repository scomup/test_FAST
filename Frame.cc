...
Frame::Frame(cv::Mat &im_, const double &timeStamp, ORBextractor* extractor,cv::Mat &K, cv::Mat &distCoef)
    :mpORBextractor(extractor), im(im_),mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone())
{
    // Exctract ORB  
