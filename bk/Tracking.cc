...
{
    std::ofstream ofs;
	ofs.open( "/home/liu/ii.txt",ios::app );
    ...
    {
        Initialize();
        ofs <<"Initialize:" <<mCurrentFrame.mTcw << std::endl;
    }
    else
    {
        ...
        if(mState==WORKING && !RelocalisationRequested())
        {
            if(!mbMotionModel || mpMap->KeyFramesInMap()<4 || mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2){
                ...
                ofs <<"TrackPreviousFrame:" <<mCurrentFrame.mTcw << std::endl;
            }
            else
            {
                bOK = TrackWithMotionModel(ofs);
                ofs <<"TrackWithMotionModel:" <<mCurrentFrame.mTcw << std::endl;
                if(!bOK){
                    bOK = TrackPreviousFrame();
                    ofs <<"TrackPreviousFrame:" <<mCurrentFrame.mTcw << std::endl;
                }
...
bool Tracking::TrackWithMotionModel(std::ofstream &ofs)
{
    ORBmatcher matcher(0.9,true);
    vector<MapPoint*> vpMapPointMatches;
    ...
    ofs <<"mCurrentFrame.mTcw" <<mCurrentFrame.mTcw<<std::endl;
    fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));

    // Project points seen in previous frame
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,15);
    ofs <<"nmatches"<< nmatches <<std::endl;
    if(nmatches<20)
       return false;

    // Optimize pose with all correspondences
    Optimizer::PoseOptimization(&mCurrentFrame, ofs);
    ofs <<"opt mCurrentFrame.mTcw"<< mCurrentFrame.mTcw <<std::endl;
    // Discard outliers
